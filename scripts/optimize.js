...
function resize(sourceFile, destFile) {
  return sharp(sourceFile)
    ...
    .toBuffer()
    .then(buffer => Promise.fromCallback(cb => fs.writeFile(destFile, buffer, cb)))
    .then(function() {
    ...
    .catch(function(err) {
      ...
      errorList[sourceFile] = { message: err.message, fileName: err.fileName, lineNumber: err.lineNumber };
    });
}

function makefilename(filepath) {
  const hash = crypto.createHash('md5').update(basename(filepath)).digest('hex');
